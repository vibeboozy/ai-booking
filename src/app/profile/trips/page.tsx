...
export default async function ProfileTripsPage({ searchParams }: ProfileTripsPageProps) {
  ...
  const { upcoming: upcomingTrips, history: historyTrips } = await getAllUserTrips(session.user.id);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <main className="mx-auto max-w-3xl px-4 py-8">
        ...
        <nav className="mb-6 flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>/</span>
          <span className="text-gray-900 dark:text-gray-200">Мои поездки</span>
        </nav>
