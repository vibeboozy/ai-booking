...
describe('useTrips', () => {
  ...
  describe('fetch trips', () => {
    it('вызывает fetch с правильным URL для upcoming', async () => {
      ...
      await waitFor(() => {
        expect(mockFetch).toHaveBeenCalledWith('/api/profile/trips?status=upcoming');
      });
    });
    ...
    it('вызывает fetch с правильным URL для history', async () => {
      ...
      await waitFor(() => {
        expect(mockFetch).toHaveBeenCalledWith('/api/profile/trips?status=history');
      });
    });
    ...
    it('вызывает fetch без параметров когда status не передан', async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockFetch).toHaveBeenCalledWith(expect.stringMatching(/^\/api\/profile\/trips\??$/));
      });
    });
