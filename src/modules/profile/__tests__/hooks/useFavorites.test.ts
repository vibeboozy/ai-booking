...
describe('useFavorites', () => {
  ...
  describe('initial state', () => {
    ...
    it('использует initialFavoriteIds если передан', () => {
      ...
      const { result } = renderHook(() => useFavorites({ initialFavoriteIds: initialIds }));
  ...
  describe('toggle', () => {
    ...
    it('вызывает onError при неудачном DELETE', async () => {
      ...
      const { result } = renderHook(() => useFavorites({ initialFavoriteIds: initialIds, onError }));
  ...
  describe('fetch favorites on mount', () => {
    ...
    it('делает fetch если initialFavoriteIds пустой', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ data: [{ id: 'listing-1' }, { id: 'listing-2' }] }),
      });
