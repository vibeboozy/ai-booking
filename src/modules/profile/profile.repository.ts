 ...
 */
function mapBookingStatus(status: BookingStatus): 'pending' | 'confirmed' | 'completed' | 'cancelled' {
  const statusMap: Record<BookingStatus, 'pending' | 'confirmed' | 'completed' | 'cancelled'> = {
    PENDING: 'pending',
    CONFIRMED: 'confirmed',
    COMPLETED: 'completed',
    CANCELLED: 'cancelled',
...
export const getUserTrips = cache(async (
  ...
  return bookings.map((booking) => ({
    ...
    listing: {
      ...
      propertyType: booking.listing.propertyType.toLowerCase() as ListingPreview['propertyType'],
    },
    canReview:
 ...
 */
export const getAllUserTrips = cache(async (userId: string): Promise<UserTripsResult> => {
  const now = new Date();
  now.setHours(0, 0, 0, 0);
  ...
  history.sort((a, b) => new Date(b.booking.checkIn).getTime() - new Date(a.booking.checkIn).getTime());
 ...
 */
export const getUserFavorites = cache(async (userId: string): Promise<ListingPreview[]> => {
  ...
  return favorites.map((fav) => ({
    ...
    propertyType: fav.listing.propertyType.toLowerCase() as ListingPreview['propertyType'],
  }));
});
