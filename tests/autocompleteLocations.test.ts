...
describe('autocompleteLocations', () => {
  ...
  it('SC-001: q = empty string returns all/popular locations up to limit', async () => {
    const mockLocations = [
      { id: '1', name: 'Sochi', type: 'CITY', slug: 'sochi', lat: 43.6, lng: 39.7 },
      { id: '2', name: 'Moscow', type: 'CITY', slug: 'moscow', lat: 55.75, lng: 37.62 },
    ];
  ...
  it('SC-003: q = 2+ chars case-insensitive returns matching locations', async () => {
    ...
    expect(result).toContainEqual(expect.objectContaining({ name: 'Sochi', type: 'city' }));
  });

  it('SC-004: limit parameter restricts result count', async () => {
    vi.mocked(prisma.location.findMany).mockResolvedValue([
      { id: '1', name: 'Madrid', type: 'CITY', slug: 'madrid', lat: 40.4, lng: -3.7 },
      { id: '2', name: 'Mallorca', type: 'CITY', slug: 'mallorca', lat: 39.7, lng: 2.8 },
    ] as never);
  ...
  it('SC-005: results sorted by type asc, name asc', async () => {
    const mockLocations = [
      { id: '1', name: 'Marseilles', type: 'CITY', slug: 'marseilles', lat: 43.3, lng: 5.4 },
      { id: '2', name: 'France', type: 'COUNTRY', slug: 'france', lat: 46.2, lng: 2.2 },
    ];
    vi.mocked(prisma.location.findMany).mockResolvedValue(mockLocations as never);
